  ...
  return (
    <div className="exit-explanation">
      <div className="trigger" onClick={() => setExpanded(!expanded)}>
        Почему квиз на весь экран?
      </div>
      <div style={{ display: expanded ? "block" : "none" }}>
        Смысл квиза в том, чтобы проверить, что осталось в голове без подсказки.
        Постарайтесь отвечать, не подглядывая в текст: если вопрос не даётся —
        это и есть полезный результат, значит раздел стоит перечитать.
      </div>
    </div>
  );
};
...
/** Выбирает форму слова по числу: 1 вопрос, 2 вопроса, 5 вопросов. */
let plural = (n: number, forms: [string, string, string]): string => {
  let n10 = n % 10;
  let n100 = n % 100;
  ...
  return (
    <>
      ...
      <p>
        Правильных ответов:{" "}
        <strong>
          {nCorrect} из {quiz.questions.length}
        </strong>
        .
      </p>
      {confirm}
